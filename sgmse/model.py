...
class ScoreModel(pl.LightningModule):
    def __init__(self,
        backbone: str = "ncsnpp", ode: str = "flowmatching",
        ...
        Args:
            ...
            ode: The SDE to use for the diffusion.
    ...
    def forward(self, x, t, y, ode_input, **kwargs):
    ...
    def _step(self, batch, batch_idx):
        ...
        t = torch.rand(x.shape[0], device=x.device) * (self.ode.T - self.t_eps) + self.t_eps
        ...
        ode_input = y
        score = self(perturbed_data, t, y,ode_input)
        err = score - (self.ode.der_mean(x,t,y)+self.ode.der_std(t)*z)
        loss = self._loss(err)
        return loss
    ...
    def get_ode_sampler(self, y, N=5, minibatch=1, **kwargs):
        ...
        if minibatch is None:
            return sampling.get_ode_sampler(ode, self, y=y, N=N,  **kwargs)
        else:
            M = y.shape[0]
            def batched_sampling_fn():
                ...
                for i in range(int(ceil(M / minibatch))):
                    ...
                    sampler = sampling.get_ode_sampler(ode, self, y=y_mini, N=N, **kwargs)
                    sample, n = sampler()
    ...
    def enhance(self, y, sampler_type='ode', predictor="reverse_diffusion",
...
class StochasticRegenerationModel(pl.LightningModule):
    def __init__(self,
        backbone_denoiser: str, backbone_score: str, ode: str,
        lr: float = 1e-4, ema_decay: float = 0.999,
    ...
    def forward_score(self, x, t, score_conditioning, ode_input, **kwargs):
        dnn_input = torch.cat([x] + score_conditioning, dim=1) #b,n_input*d,f,t
        score = -self.score_net(dnn_input, t)
        std = self.ode._std(t)
        if std.ndim < ode_input.ndim:
            std = std.view(*std.size(), *((1,)*(ode_input.ndim - std.ndim)))
        return score
    ...
    def _step(self, batch, batch_idx):
        ...
        ode_target = x
        ode_input = y_denoised
        # Forward process
        t = torch.rand(x.shape[0], device=x.device) * (self.ode.T - self.t_eps) + self.t_eps
        mean, std = self.ode.marginal_prob(ode_target, t, ode_input)
        ...
        score = self.forward_score(perturbed_data, t, score_conditioning, ode_input)
        err = score - (self.ode.der_mean(ode_target,t,ode_input)+self.ode.der_std(t)*z)
    ...
    def get_ode_sampler(self, y, N=5, minibatch=1, **kwargs):
        ...
        kwargs = {"eps": self.t_eps,  **kwargs}
        if minibatch is None:
            return sampling.get_ode_sampler(ode, self.forward_score, y=y, N=N,  **kwargs)
        else:
            M = y.shape[0]
            def batched_sampling_fn():
                samples, ns = [], []
                for i in range(int(ceil(M / minibatch))):
                    y_mini = y[i*minibatch:(i+1)*minibatch]
                    sampler = sampling.get_ode_sampler(ode, self.forward_score, y=y_mini, N=N, **kwargs)
                    sample, n = sampler()
                    samples.append(sample)
                    ns.append(n)
                samples = torch.cat(samples, dim=0)
    ...
    def enhance(self, y, sampler_type="ode", predictor="reverse_diffusion",
        corrector="none", N=1, corrector_steps=1, snr=0.5, timeit=False,
        ...
        with torch.no_grad():
            ...
            if self.score_net is not None and not denoiser_only:
                ...
                if self.condition == "noisy":
                    ...
                    raise NotImplementedError(f"Don't know the conditioning you have wished for: {self.condition}")
                kwargs = {"conditioning":score_conditioning, **kwargs}
                # Reverse process
                if sampler_type == "ode":
                    sampler = self.get_ode_sampler(Y_denoised, N=N, **kwargs)
                else:
                    print("{} is not a valid sampler type!".format(sampler_type))
                sample, nfe = sampler()
            else:
